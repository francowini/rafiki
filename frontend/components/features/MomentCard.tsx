...
export function MomentCard({ moment, onClick, onEdit, onDelete }: MomentCardProps) {
  ...
  const situationPreview =
    moment.situation.length > 100 ? moment.situation.slice(0, 100) + '...' : moment.situation;
  ...
  return (
    <Card className="hover:shadow-md transition-shadow cursor-pointer" onClick={onClick}>
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          <div className="space-y-1">
            <CardTitle className="text-base font-medium">{dateStr}</CardTitle>
            <CardDescription className="text-sm">{timeStr}</CardDescription>
          </div>
      ...
      <CardContent className="space-y-3">
        <p className="text-sm text-muted-foreground line-clamp-3">{situationPreview}</p>
