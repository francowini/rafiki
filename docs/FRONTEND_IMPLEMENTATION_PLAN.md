...
### Task 1.1: Fix CORS Configuration (CRITICAL)

**File:** [api/services/partners/main.go](../api/services/partners/main.go)

**Current Code (Line 220-222):**
```go
...
webAPI := mux.WebAPI(cfgMux,
    buildRoutes(),
    mux.WithCORS(cfg.Web.CORSAllowedOrigins),  // ← Add this line
)
```
...
nano api/services/partners/main.go

# 2. Add the WithCORS option (see above)

# 3. Commit the change
git add api/services/partners/main.go
git commit -m "fix: wire CORS configuration to mux"

# 4. Push to repository
git push origin main
```

### Task 1.2: Update CORS for Development
...
# AFTER (for development with localhost):
- PARTNER_WEB_CORSALLOWEDORIGINS=http://localhost:3000,http://localhost:3001,*
```

**Note:** The `*` wildcard should be removed in production.

### Task 1.3: Deploy Backend Changes
...
curl -i -X OPTIONS http://178.156.170.37:3000/v1/thinks \
...
```bash
# Test GET /v1/thinks
curl http://178.156.170.37:3000/v1/thinks

# Test POST /v1/thinks
curl -X POST http://178.156.170.37:3000/v1/thinks \
  -H "Content-Type: application/json" \
  -d '{"category":"personal","content":"Test think"}'

# Test GET /v1/thinks/{id} (use ID from previous response)
curl http://178.156.170.37:3000/v1/thinks/{id}

# Test health endpoints
curl http://178.156.170.37:3000/v1/readiness
curl http://178.156.170.37:3000/v1/liveness
```

### Acceptance Criteria

- [ ] CORS configuration wired to mux
...
### Task 2.1: Set Up DNS Records

**Domain Provider:** (e.g., Namecheap, Cloudflare, Google Domains)

**Required DNS Records:**

| Type | Name | Value | TTL |
|------|------|-------|-----|
| A | api.rafiki.com | 178.156.170.37 | 300 |
| A | grafana.rafiki.com | 178.156.170.37 | 300 |
| CNAME | app.rafiki.com | cname.vercel-dns.com | 300 |

**Note:** The Vercel CNAME will be provided by Vercel after domain configuration.

**Steps:**
```bash
# 1. Log into your DNS provider
# 2. Add A records for api.rafiki.com and grafana.rafiki.com
# 3. Wait for DNS propagation (5-30 minutes)

# 4. Verify DNS propagation
dig api.rafiki.com
# Should return: 178.156.170.37

nslookup api.rafiki.com
...
mkdir -p /opt/rafiki/nginx
mkdir -p /opt/rafiki/certbot/conf
mkdir -p /opt/rafiki/certbot/www

# 3. Create nginx configuration file
nano /opt/rafiki/nginx/nginx.conf
```

**Nginx Configuration File:**

```nginx
# /opt/rafiki/nginx/nginx.conf
...
# Upstream backend
upstream rafiki_backend {
    server rafiki-service:3000;
    keepalive 32;
}
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/api.rafiki.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/api.rafiki.com/privkey.pem;
    ...
    location ~ ^/v1/(readiness|liveness)$ {
        proxy_pass http://rafiki_backend;
...
server {
    listen 80;
    server_name grafana.rafiki.com;
...
server {
    ...
    ssl_certificate /etc/letsencrypt/live/grafana.rafiki.com/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/grafana.rafiki.com/privkey.pem;
    ...
    location / {
        proxy_pass http://10.10.0.21:3100;
        proxy_set_header Host $host;
        proxy_set_header X-Real-IP $remote_addr;
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
...
```bash
# On server
nano /opt/rafiki/docker-compose.yml

# Add the following services at the end:
```
...
### Task 2.4: Obtain SSL Certificates

**Important:** Wait until DNS propagation is complete before running this step.

```bash
# On server, temporarily start only nginx (without SSL first)
# Comment out the SSL lines in nginx.conf temporarily, or create a simplified version

# Start nginx for HTTP challenge
docker compose up -d nginx

# Obtain SSL certificate for api.rafiki.com
...
# Obtain SSL certificate for grafana.rafiki.com (optional)
docker compose run --rm certbot certonly --webroot \
  --webroot-path=/var/www/certbot \
  --email your-email@example.com \
  --agree-tos \
  --no-eff-email \
  -d grafana.rafiki.com

# Verify certificates were created
ls -la /opt/rafiki/certbot/conf/live/
...
### Task 2.5: Update Firewall Rules

**Security improvement: Close direct API port access**
...
# REMOVE direct API port exposure (security improvement)
ufw delete allow 3000/tcp

# Block PostgreSQL from external access
ufw deny 5432/tcp

# Block Grafana/Tempo from external access (use nginx proxy instead)
ufw deny 3100/tcp
ufw deny 3200/tcp
ufw deny 4317/tcp
...
### Task 2.6: Update Production CORS

**File:** `/opt/rafiki/.env` on server

```bash
# Edit .env file
nano /opt/rafiki/.env

# Update CORS to specific domains (replace * with actual domains)
PARTNER_WEB_CORSALLOWEDORIGINS=https://app.rafiki.com,https://rafiki.com,http://localhost:3000

# Save and exit
```

### Task 2.7: Deploy Infrastructure Changes

```bash
# On server
cd /opt/rafiki

# Pull latest code (includes CORS fix from Phase 1)
git pull origin main

# Deploy with new nginx configuration
docker compose down
...
curl https://api.rafiki.com/v1/thinks
```
...
- [ ] Firewall rules updated (port 3000 closed)
- [ ] API accessible via https://api.rafiki.com
